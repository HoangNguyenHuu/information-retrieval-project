...
public class Evaluation {
    public static void main(String[] args) throws ParseException, InvalidTokenOffsetsException, IOException {
        ...
        PrintWriter writer = new PrintWriter("evaluation_results/result.txt");
    ...
    public static int get_number_result_true_system(JsonArray relevances, JsonArray docs, int query_id, int total_relevance_this_query, String query) throws IOException {
        ...
        PrintWriter writer = new PrintWriter(new FileWriter("evaluation_results/query" + query_id + ".txt"));
...
//                System.out.println("Query id: " + query_id + " - doc id: " + doc_id + " - relevance: " + relevance);
            ...
            writer.println("Số kết quả trả về: " + String.format("%4d", j + 1) + " - precision@"+ (j+1)+": " + String.format("%.3f", precision_i) + " - recall@" +(j+1) +": " + String.format("%.3f", recall_i) + " - f1 measure:  " + String.format("%.3f", f1));
            if (j == docs.size() - 1) {
                writer.println();
